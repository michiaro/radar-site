<template lang="pug">
  .wrapper(v-if="isReady")
    .block
      .work-top-block(
        :style="{'backgroundImage': 'url(http://new.radar-online.mcdir.ru' +this.content.works[workIndex].header.path+')'}"
      )
      .content.work-top-block__content 
        .slider-content
          h1.slider-content__title(
            :class=" { 'dark' : this.content.works[workIndex].isInverse }"
          ) {{ this.content.works[workIndex].title}}
          p.slider-content__text(
            :class=" { 'dark' : this.content.works[workIndex].isInverse }"
          ) {{ this.content.works[workIndex].prescription}}
        .work-top-block__socials
          a.work-top-block__social-link.work-top-block__social-link--insta(
            :href="this.content.works[workIndex].instagram",
            target="_blank"
          )
            svg(
              width="30",
              ...
              path(
                :fill="this.content.works[workIndex].socialsColor",
          ...
          a.work-top-block__social-link.work-top-block__social-link--fb(
            :href="this.content.works[workIndex].facebook",
            target="_blank"
          )
            svg(
              width="30",
          ...
          a.work-top-block__social-link.work-top-block__social-link--vk(
            :href="this.content.works[workIndex].vkontakte",
            target="_blank"
          )
            svg(
              width="30",
        ...
        .work-top-block__timing(
          :class=" { 'dark' : this.content.works[workIndex].isInverse }"
        ) {{ this.content.works[workIndex].dates }}
    .block.work-zone-block
      .content.work-zone__content
        .work-zone__text-block(
          ...
          div(
            v-html="this.content.works[workIndex].about",
          )
        ...
        .work-zone__slider
          img(
            :src="'http://new.radar-online.mcdir.ru'+this.content.works[workIndex].logo.path", :alt="this.content.works[workIndex].title+ ' Логотип проекта'"
          )
        #work-layout(
          v-html="this.content.works[workIndex].layout"
        ) 
        .work-zone__low-part
          .who-did-it
            span.who-did-it__text
              .who-did-it__person(v-for="member in this.content.works[workIndex].credits") 
          ...
          .share-block
            .tags
              span.tags-link(
                v-for="tag in this.content.works[workIndex].tags",
                :key="tag._id",
      ...
      .content.company-description-content
        .company-description__img
          img(
            ...
            :alt="this.content.works[workIndex].client.display+ 'Логотип'",
            @click="setClientFilter(content.works[workIndex].client._id)"
          )
        p.company-description__text(
          v-html="this.content.works[workIndex].clientAbout"
        ) 
    router-link.next-work(
      v-if="nextWork > -1",
      :to="'/all-works/' +this.content.works[nextWork].slug"
    )
      .next-work__cover(
        :style="{'backgroundImage': 'url(http://new.radar-online.mcdir.ru' +this.content.works[nextWork].header.path+')'}"
      ) 
      .content.next-work__content(
        :class=" { 'dark' : this.content.works[nextWork].isInverse }"
      )
        .next-work__title Следующий проект
          .next-work__arrow(
            :class=" { 'next-work__arrow_dark' : this.content.works[nextWork].isInverse }"
          )
        div
          .next-work__content_title {{ this.content.works[nextWork].title }}
          p.next-work__content_text {{ this.content.works[nextWork].prescription}}
</template>
...
export default {
  ...
  methods: {
    ...
    clientLogo(id) {
      ...
      return currentClient.logo.path;
    },
    setClientFilter(id) {
      var currentClient = this.content.clients.find(
        client => client._id === id
